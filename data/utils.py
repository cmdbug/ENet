...
def get_image_from_prediction(predictions):
    ...
    imagex = torchvision.utils.make_grid(color_predictions).numpy()
...
def get_image_from_label(label):
    ...
    # _, predictionx = torch.max(label.detach(), 1)
    transf_slices = [label_to_rgb(tensor) for tensor in torch.unbind(label)]
    color_predictions = torch.stack(transf_slices)
    labelx = torchvision.utils.make_grid(color_predictions).numpy()
